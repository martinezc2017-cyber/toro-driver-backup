...
class StatePricing {
  ...
  factory StatePricing.fromJson(Map<String, dynamic> json) {
    return StatePricing(
      ...
      countryCode: json['country_code']?.toString() ?? 'US',
      baseFare: (json['ride_base_fare'] as num?)?.toDouble() ?? 0,
      perMileRate: (json['ride_per_km'] as num?)?.toDouble() ?? 0,
      perMinuteRate: (json['ride_per_minute'] as num?)?.toDouble() ?? 0,
      minimumFare: (json['ride_minimum_fare'] as num?)?.toDouble() ?? 0,
      bookingFee: 0.0, // Column doesn't exist in pricing_config
      serviceFee: 0.0, // Column doesn't exist in pricing_config
      cancellationFee: (json['ride_cancellation_fee'] as num?)?.toDouble() ?? 0,
      driverPercentage: (json['driver_commission'] as num?)?.toDouble() ?? 0,
      platformPercentage: (json['platform_commission'] as num?)?.toDouble() ?? 0,
      insurancePercentage: (json['insurance_percent'] as num?)?.toDouble() ?? 0,
      taxPercentage: (json['tax_percent'] as num?)?.toDouble() ?? 0,
      peakMultiplier: (json['surge_peak'] as num?)?.toDouble() ?? 1.0,
      nightMultiplier: (json['surge_night'] as num?)?.toDouble() ?? 1.0,
      weekendMultiplier: (json['surge_weekend'] as num?)?.toDouble() ?? 1.0,
      ...
      qrPointValue: (json['qr_point_value'] as num?)?.toDouble() ?? 0,
      tncTaxPerTrip: 0.0, // Column doesn't exist in pricing_config
  ...
  Map<String, dynamic> toSnapshot() {
    return {
      ...
      'ride_base_fare': baseFare,
      'ride_per_km': perMileRate,
      'ride_per_minute': perMinuteRate,
      'ride_minimum_fare': minimumFare,
      // booking_fee and service_fee don't exist in pricing_config
      'ride_cancellation_fee': cancellationFee,
      'driver_commission': driverPercentage,
      'platform_commission': platformPercentage,
      'insurance_percent': insurancePercentage,
      'tax_percent': taxPercentage,
      'surge_peak': peakMultiplier,
      'surge_night': nightMultiplier,
      'surge_weekend': weekendMultiplier,
      'carpool_discount_per_seat': carpoolDiscountPerSeat,
      'carpool_max_discount': carpoolMaxDiscount,
      'qr_point_value': qrPointValue,
      // tnc_tax_per_trip doesn't exist in pricing_config
